...
import com.github.liachmodded.networking.mixin.access.LoginQueryResponseC2SPacketAccess;
...
public final class ServerLoginNetworkAddon extends AbstractNetworkAddon<LoginC2SContext> {
	...
	public ServerLoginNetworkAddon(ServerLoginNetworkHandler handler) {
		...
		this.queryIdFactory = CartNetworkingDetails.createQueryIdManager();
	}
	...
	private boolean handle(int queryId, PacketByteBuf originalBuf) {
		...
		if (channel == null) {
			CartNetworkingDetails.LOGGER.warn("Query ID {} was received but no channel has been associated in {}!", queryId, this.connection);
			return false;
		}
